...
class FFD {
    ...
    rebuildLattice(bbox, span_counts) {
        ...
        for (let i = 0; i < this.mCtrlPtCounts[0]; i++) {
            for (let j = 0; j < this.mCtrlPtCounts[1]; j++) {
                for (let k = 0; k < this.mCtrlPtCounts[2]; k++) {
                    const position = new THREE.Vector3(this.mBBox.min.x + (i / this.mSpanCounts[0]) * this.mAxes[0].x, this.mBBox.min.y + (j / this.mSpanCounts[1]) * this.mAxes[1].y, this.mBBox.min.z + (k / this.mSpanCounts[2]) * this.mAxes[2].z);
                    this.setPositionTernary(i, j, k, position);
                }
            }
        }
    }
    evalTrivariate(s, t, u) {
    ...
    setPositionTernary(i, j, k, position) {
