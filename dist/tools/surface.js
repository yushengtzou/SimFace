...
// 曲線擬合
export function createCurve(points, scene, colors) {
...
function isPointInMeshVertices(point, meshVertices) {
    return meshVertices.some(vertex => arePointsEqual(point, vertex));
}
// 用 K-D Tree 儲存網目頂點資料，以搜尋最近的頂點
export function findClosestVertices(curvePoints, meshGeometry) {
...
function getFaceCentroidsFromGeometry(geometry) {
    const centroids = [];
    const positionAttribute = geometry.getAttribute('position');
    for (let i = 0; i < positionAttribute.count; i += 3) {
        const v0 = new THREE.Vector3(positionAttribute.getX(i), positionAttribute.getY(i), positionAttribute.getZ(i));
        const v1 = new THREE.Vector3(positionAttribute.getX(i + 1), positionAttribute.getY(i + 1), positionAttribute.getZ(i + 1));
        const v2 = new THREE.Vector3(positionAttribute.getX(i + 2), positionAttribute.getY(i + 2), positionAttribute.getZ(i + 2));
...
export function findClosestFaces(curvePoints, meshGeometry) {
    if (!meshGeometry) {
    ...
    const closestFaceIndices = curvePoints.map(point => {
        const nearest = kdTreeInstance.nearest([point.x, point.y, point.z], 1)[0][0];
        const nearestVector = new THREE.Vector3(nearest[0], nearest[1], nearest[2]);
