...
class Drag {
    ...
    public _main: { _mouseX: number, _mouseY: number };
    public dragDir: THREE.Vector3;

    constructor(sceneObjects: any) {
                ...
                for (let i = 0; i < geometry.attributes.position.count; i++) {
                    ...
                    if (distance < 0.1) {  // Assuming MAX_CLICK_DISTANCE is 0.1
                        ...
                        geometry.attributes.position.setXYZ(
                            i,
                            vector3.x + this.dragDir.x * fallOff,
                            vector3.y + this.dragDir.y * fallOff,
                            vector3.z + this.dragDir.z * fallOff
                        );
                    }
                }
