...
function deformMesh(point: THREE.Vector3, mesh: THREE.Mesh, normal: THREE.Vector3, deformDist: number) {
    ...
    // Calculate orthogonal vector in the y direction
    const orthoVector = new THREE.Vector3(0, 1, 0).cross(normal).normalize();
    ...
    for (let i = 0; i < position.count; i++) {
        ...
        if (dist < MAX_CLICK_DISTANCE) {
            ...
            // Apply deformation along the orthogonal vector direction
            posArray[ix] = initPosArray[ix] + orthoVector.x * influence;
            posArray[iy] = initPosArray[iy] + orthoVector.y * influence;
            posArray[iz] = initPosArray[iz] + orthoVector.z * influence;
        }
    }
