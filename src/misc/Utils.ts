...
// 反轉對象的鍵值對
Utils.invert = function (obj: any): any {
    const keys = Object.keys(obj);
    const inv: any = {};
    for (let i = 0, nbkeys = keys.length; i < nbkeys; ++i) inv[obj[keys[i]]] = keys[i];
    return inv;
};

// 陣列操作函數
// 替換陣列中的元素
Utils.replaceElement = function (array: any[], oldValue: any, newValue: any): void {
...
// 從陣列中移除元素
Utils.removeElement = function (array: any[], remValue: any): void {
    for (let i = 0, l = array.length; i < l; ++i) {
        if (array[i] === remValue) {
            array[i] = array[l - 1];
            array.pop();
...
// 將兩個陣列合併
Utils.appendArray = function (array1: any[], array2: any[]): void {
    const nb1 = array1.length;
    const nb2 = array2.length;
    array1.length += nb2;
    for (let i = 0; i < nb2; ++i) array1[nb1 + i] = array2[i];
};

// 返回 true 如果數字是二的冪次
Utils.isPowerOfTwo = function (x: number): boolean {
    return x !== 0 && (x & (x - 1)) === 0;
};

// 返回最接近的二的冪次值
Utils.nextHighestPowerOfTwo = function (x: number): number {
    --x;
    for (let i = 1; i < 32; i <<= 1) x = x | (x >> i);
    return x + 1;
};

const sortFunc = function (a: number, b: number): number {
    return a - b;
};

// 對陣列進行排序並刪除重複值
Utils.tidy = function (array: any[]): void {
    array.sort(sortFunc);
...
// 兩個陣列的交集
Utils.intersectionArrays = function (a: any[], b: any[]): any[] {
    let ai = 0;
    let bi = 0;
    const result: any[] = [];
    const aLen = a.length;
...
// 獲取字節
Utils.getBytes = function (data: any, offset: number): number[] {
    return [
...
// 讀取二進制 uint32
Utils.getUint32 = function (data: any, offset: number): number {
    const b = Utils.getBytes(data, offset);
    return (b[0] << 0) | (b[1] << 8) | (b[2] << 16) | (b[3] << 24);
};

// 讀取二進制 float32
Utils.getFloat32 = function (data: any, offset: number): number {
    const b = Utils.getBytes(data, offset);
...
Utils.ab2str = function (buf: ArrayBuffer): string {
    let str = '';
...
Utils.getMemory = (function () {
    ...
    return function (nbBytes: number): ArrayBuffer {
        if (pool.byteLength >= nbBytes) return pool;
...
// 函數節流
Utils.throttle = function (func: Function, wait: number): Function {
    let result: any;
    const args: any[] = [];
    ...
    return function () {
        ...
        for (let i = 0; i < nbArgs; ++i) args[i] = arguments[i];
...
// 對 Vec3 陣列進行標準化
Utils.normalizeArrayVec3 = function (array: Float32Array, arrayOut = array): Float32Array {
...
// 將 Vec3 陣列轉換為 sRGB
Utils.convertArrayVec3toSRGB = function (array: Float32Array, arrayOut = array): Float32Array {
    for (let i = 0, l = array.length; i < l; ++i) {
...
// 將 Vec3 陣列轉換為線性空間
Utils.convertArrayVec3toLinear = function (array: Float32Array, arrayOut = array): Float32Array {
    for (let i = 0, l = array.length; i < l; ++i) {
...
// 計算世界座標中的頂點位置
Utils.computeWorldVertices = function (mesh: any, arrayOut?: Float32Array): Float32Array {
