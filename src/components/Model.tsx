 ...
 * @description 定義傳遞給 Model 元件的屬性介面
 *
 */
interface ModelProps {
    ...
    deformDistance: number;  // 用於形變操作的距離參數
 ...
 * @description 這個 React 函式元件負責載入 3D 模型，並在場景中進行渲染和互動操作
 *
 */
const Model: React.FC<ModelProps> = ({ 
    ...
    // 使用 useEffect Hook 在模型載入後執行回調，並列印模型資訊
    useEffect(() => {
        if (obj) {
            onLoad();
            obj.traverse((child) => {
        ...
        if (intersects.length > 0) {
            ...
            setSelectedPoints((prevPoints) => {
                ...
                if (updatedPoints.length === 2) {
                    ...
                    const geometry = createTubeGeometry(updatedPoints, 0.005); // 0.05 是管道的半径，可調整此值
                    ...
                    const sprite = createTextSprite((distance.toFixed(2) * 5) + ' cm');
